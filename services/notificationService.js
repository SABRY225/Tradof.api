...
const notificationService = {
    sendNotification: async (req, res) => {
        try {
            ...
            const newNotification = new Notification({ type, receiverId, message,description });
    ...
    SettingNotification: async (req, res) => {
        try {
            ...
            if (sendEmail === undefined || alertOffers === undefined || messageChat === undefined) {
                return res.status(400).json({ success: false, message: 'sendEmail, alertOffers, and messageChat are required!' });
            }
            ...
            const updatedSetting = await SettingNotification.findOneAndUpdate(filter, update, options);
    ...
    getSettingNotification:async (req,res) => {
       try{
        ...
        if(companyId){
            const settingNotification = await SettingNotification.findOne({companyId});
            if(!settingNotification){
                return res.status(404).json({ success: false, message: 'Notification setting not found' });
            }
            res.status(200).json({
                success: true,
                data: {alertOffers:settingNotification.alertOffers,
                    messageChat:settingNotification.messageChat,
                    sendEmail:settingNotification.sendEmail
                 }
            });
        }else{
            const settingNotification = await SettingNotification.findone({'user.id':user.id});
            if(!settingNotification){
                return res.status(404).json({ success: false, message: 'Notification setting not found' });
            }
            res.status(200).json({
